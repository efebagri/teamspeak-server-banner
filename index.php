 ...
 * index.php
 *
 * Author: Dennis Abrams (Code improvements by Efe Bagri)
 * Orginal Repository: https://github.com/dennisabrams/teamspeak3-banner
 * Improved repository: https://github.com/efebagri/teamspeak3-banner
 *
 */
...
if (file_exists('vendor/autoload.php')) {
    ...
    die("Vendor autoload file not found.<br>Please run: composer install");
}

require_once('config.php');
require_once('drawing.php');

// No direct include for the TeamSpeak3 library is necessary
// Ensure that the TeamSpeak3 class is available
if (!class_exists('PlanetTeamSpeak\TeamSpeak3Framework\TeamSpeak3')) {
    die("The TeamSpeak3 class is not available.<br>Please run: composer require planetteamspeak/ts3-php-framework");
}

if (!class_exists('\PlanetTeamSpeak\TeamSpeak3Framework\Helper\Convert')) {
    die("Class 'TeamSpeak3_Helper_Convert' not found.");
}
...
try {
	...
	$ts3 = \PlanetTeamSpeak\TeamSpeak3Framework\TeamSpeak3::factory('serverquery://' .rawurlencode($serverquery_username) .':' .rawurlencode($serverquery_password) ."@$server_ip:$serverquery_port/?server_port=$server_port");

	// Get server Info
	$server_name = $ts3->virtualserver_name;
	$server_uptime = \PlanetTeamSpeak\TeamSpeak3Framework\Helper\Convert::seconds($ts3->virtualserver_uptime);
...
// Serverquery connection failed
	...
	error_log('Error Code: ' . $e->getCode() . ' ' . $e->getMessage());
